...
sed -i 's|^minimum-gas-prices *=.*|minimum-gas-prices = "0.15uinit,0.01uusdc"|g' $APP_TOML


# Customize ports
CLIENT_TOML=$HOME/.initia/config/client.toml
...
mkdir -p ~/.initia/cosmovisor/upgrades
cp ~/go/bin/initiad $HOME/.initia/cosmovisor/genesis/bin
