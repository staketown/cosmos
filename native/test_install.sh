...
ssed -i.bak -e "s%^address = \'localhost:9090\'%address = \"0.0.0.0:$PORT_GRPC\"%; s%^address = \'localhost:9091\'%address = \"0.0.0.0:$PORT_GRPC_WEB\"%; s%^address = \'tcp://localhost:1317\'%address = \"tcp://0.0.0.0:$PORT_API\"%" $APP_TOML && \
...
cp ~/go/bin/gonative $HOME/.gonative/cosmovisor/genesis/bin
