...
curl -s https://raw.githubusercontent.com/ojo-network/price-feeder/umee/umee-provider-config/deviation-thresholds.toml >> $HOME/.umee-price-feeder/config.toml
echo >> $HOME/.umee-price-feeder/config.toml
curl -s https://raw.githubusercontent.com/ojo-network/price-feeder/umee/umee-provider-config/endpoints.toml >> $HOME/.umee-price-feeder/config.toml
echo >> $HOME/.umee-price-feeder/config.toml
...
KEYRING="os"
LISTEN_PORT=7173
RPC_PORT=$(grep -A 3 "\[rpc\]" ~/.umme/config/config.toml | egrep -o ":[0-9]+" | awk '{print substr($0, 2)}')
GRPC_PORT=$(grep -A 6 "\[grpc\]" ~/.umme/config/app.toml | egrep -o ":[0-9]+" | awk '{print substr($0, 2)}')
